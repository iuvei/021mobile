var gdesf_playType = 0;
var gdesf_playMethod = 0;
var gdesf_sntuo = 0;
var gdesf_rebate;
var gdesfScroll;
...
function gdesfChangeItem(val){
    ...
    if(val == 'gdesf1'){
        ...
        var tips = "<p>格式说明<br/>"+name[gdesf_playMethod - 27]+":"+ (array[gdesf_playMethod - 27]) +"<br/>1)每注必须是"+(gdesf_playMethod - 26)+"个号码,每个号码之间以空格分割;2)每注之间以逗号、分号、换行符分割;3)只支持单式.</p>";
        createSingleLayout("gdesf",tips);
...
function gdesf_calcNotes(){
    ...
    //验证慢彩最大倍数为9999
    if($("#gdesf_beiNum").val() > 9999){
        $("#gdesf_beiNum").val(9999);
    }
