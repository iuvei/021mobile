var sdesf_playType = 0;
var sdesf_playMethod = 0;
var sdesf_sntuo = 0;
var sdesf_rebate;
var sdesfScroll;
...
function sdesfChangeItem(val){
    ...
    if(val == 'sdesf1'){
        ...
        var tips = "<p>格式说明<br/>"+name[sdesf_playMethod - 27]+":"+ (array[sdesf_playMethod - 27]) +"<br/>1)每注必须是"+(sdesf_playMethod - 26)+"个号码,每个号码之间以空格分割;2)每注之间以逗号、分号、换行符分割;3)只支持单式.</p>";
        createSingleLayout("sdesf",tips);
...
function sdesf_calcNotes(){
    ...
    //验证慢彩最大倍数为9999
    if($("#sdesf_beiNum").val() > 9999){
        $("#sdesf_beiNum").val(9999);
    }
