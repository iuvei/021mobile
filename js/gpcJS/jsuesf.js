var jsuesf_playType = 0;
var jsuesf_playMethod = 0;
var jsuesf_sntuo = 0;
var jsuesf_rebate;
var jsuesfScroll;
...
function jsuesfChangeItem(val){
    ...
    if(val == 'jsuesf1'){
        ...
        var tips = "<p>格式说明<br/>"+name[jsuesf_playMethod - 27]+":"+ (array[jsuesf_playMethod - 27]) +"<br/>1)每注必须是"+(jsuesf_playMethod - 26)+"个号码,每个号码之间以空格分割;2)每注之间以逗号、分号、换行符分割;3)只支持单式.</p>";
        createSingleLayout("jsuesf",tips);
...
function jsuesf_calcNotes(){
    ...
    //验证慢彩最大倍数为9999
    if($("#jsuesf_beiNum").val() > 9999){
        $("#jsuesf_beiNum").val(9999);
    }
