var ahesf_playType = 0;
var ahesf_playMethod = 0;
var ahesf_sntuo = 0;
var ahesf_rebate;
var ahesfScroll;
...
function ahesfChangeItem(val){
	...
	if(val == 'ahesf1'){
		...
		var tips = "<p>格式说明<br/>"+name[ahesf_playMethod - 27]+":"+ (array[ahesf_playMethod - 27]) +"<br/>1)每注必须是"+(ahesf_playMethod - 26)+"个号码,每个号码之间以空格分割;2)每注之间以逗号、分号、换行符分割;3)只支持单式.</p>";
		createSingleLayout("ahesf",tips);
...
function ahesf_calcNotes(){
	...
	//验证慢彩最大倍数为9999
	if($("#ahesf_beiNum").val() > 9999){
		$("#ahesf_beiNum").val(9999);
	}
