...
function searchResult(){
	if(historyLot==50){
		...
		ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"GetLotteryDrawPageFlx","CurrentPageSize":15,"LotteryCode":"50","BeginTime":"' + startDateTime + '","ProjectPublic_PlatformCode":2,"EndTime":"' + endDateTime + '","CurrentPageIndex":'+ page_mmc +'}',MmcHistorySuccessCallBack, '正在加载数据...');
	}else{
		ajaxUtil.ajaxByAsyncPost1(null, '{"IsSelf":false,"ProjectPublic_PlatformCode":2,"reType":4,"InterfaceName":"GetHisNumber","Size":'+PAGESIZE+',"Page":'+historyAwardPage+',"CZID":' +  historyLot + '}',successCallBackhistory, '正在加载数据...');
	}
...
function MmcHistorySuccessCallBack(data){
	...
	if(data.SystemState == -1){
		loginAgain();
	}else if(data.SystemState == 64){
		var info=data.IssueModlst;
