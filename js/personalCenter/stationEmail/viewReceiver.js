...
function getAllReceiver() {
    page = 0;
    var param = '{"ProjectPublic_PlatformCode":2,"InterfaceName":"GetAllReceivePerson","Sendid":'+ messageID +',"CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_viewRec + '}';
...
//@ 获取下级收件人
function getAllReceCallBack(data) {
    if (page == 0) {
        $("#viewLists").empty();
    ...
    if (data.SystemState == 64){
        var receiver = data.StationEmailReceiveBoxs;
        isHasMorePage(receiver,PAGESIZE_viewRec);
        $.each(receiver,function (key,val) {
            var recName = val.ReceivePerson;
