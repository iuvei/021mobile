		...
		isHasMorePage(btInfo,PAGESIZE_myBetting);
	 	for (var i = 0; i < btInfo.length; i++) {
			...
			var $itemLi = $('<li></li>').data('scheme',scheme);
				...
				$itemLi.append('<a class="recordList"><dl class="orderList"><dd>期号:&nbsp;' + dataSet.IssueNumber + '</dd><dd>投注金额:&nbsp;<span class="red">' + dataSet.BetMoney +'元</span></dd><dd>彩种:&nbsp;'+LotteryInfo.getLotteryNameById(dataSet.BetTb.toString())+'</dd><dd>时间:&nbsp;' + dataSet.InsertTime +'</dd></dl></a>');
...
function nextPage_Betting(startDateTime, endDateTime, lotteryType) {
    var params = '{"InterfaceName":"GetBetDataListFlex","SourceCode":1,"ProjectPublic_PlatformCode":2,"LotteryCode":' + lotteryType + ',"IsHistory":' + IsHistory + ',"insertTimeMin":"' + startDateTime + '","insertTimeMax":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_myBetting + '}';
