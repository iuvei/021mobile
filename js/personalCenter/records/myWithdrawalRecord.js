    ...
    if (data.SystemState == 64   && data.DataCount !=0) {
        ...
        for (var i = 0; i < info.length; i++) {
			...
			var $itemLi = $('<li></li>').data('withdrawal',dataSet);
				...
				$itemLi.append('<a class="recordList"><dl class="orderList"><dd>账户:&nbsp;' + dataSet.accountnum + '</dd><dd>金额:&nbsp;<span class="red">' + dataSet.tmoney +'元</span></dd><dd>状态:&nbsp;'+dataSet.state+'</dd><dd>时间:&nbsp;' + dataSet.optime +'</dd></dl></a>');
...
function searchChargeWit(startDateTime, endDateTime, type) {
    page=0;
    var paramssearch = '{"InterfaceName":"GetUserDrawingsInfoByPage","ProjectPublic_PlatformCode":2,"DrawingsState":' + type + ',"BeginDate":"' + startDateTime + '","EndDate":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_myWith + '}';
