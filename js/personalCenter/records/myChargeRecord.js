...
function searchSuccessCallBackmyCharge(data) {
    ...
    if (data.SystemState == 64  && data.DataCount !=0) {
        ...
        for (var i = 0; i < info.length; i++) {
            ...
            if(2==info[i].RechargeType){
                dataSet.payType = "其他";
            }else if(3==info[i].RechargeType){
                dataSet.payType = "人工存款";
            }else if(4==info[i].RechargeType){
                dataSet.payType = "活动";
            }else {
                var temp = rechargeTypesObj['online_type_' + info[i].RechargeType];
                var allRecType = jsonUtils.toJson(localStorageUtils.getParam("allRecType"));
                for (var j = 0; j < allRecType.length; j++) {
                    if (temp == allRecType[j].Pay) {
                        dataSet.payType = allRecType[j].PayName;    //银行名称
                    }
                }
            }
            ...
            $itemLi.append('<a class="recordList"><dl class="orderList"><dd>账户:&nbsp;' + dataSet.accountnum + '</dd><dd>金额:&nbsp;<span class="red">' + dataSet.tmoney +'元</span></dd><dd>类型:&nbsp;'+dataSet.payType+'</dd><dd>时间:&nbsp;' + dataSet.optime +'</dd></dl></a>');
...
function searchCharge(startDateTime, endDateTime, type) {
    page=0;
    var paramssearch = '{"InterfaceName":"GetUserRechargeInfoByPage","ProjectPublic_PlatformCode":2,"RechargeState":' + type + ',"BeginDate":"' + startDateTime + '","EndDate":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_myCharge + '}';
...
function getMyChargeRecords_scroll(startDateTime, endDateTime, type){
    var paramssearch_scroll = '{"InterfaceName":"GetUserRechargeInfoByPage","ProjectPublic_PlatformCode":2,"RechargeState":' + type + ',"BeginDate":"' + startDateTime + '","EndDate":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_myCharge + '}';
