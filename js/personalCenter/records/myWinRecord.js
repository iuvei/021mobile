		...
		localStorageUtils.setParam("sourceFlag", "1");
	 	for (var i = 0; i < btInfo.length; i++) {
			...
			var $itemLi = $('<li></li>').data('scheme',scheme);
				...
				$itemLi.append('<a class="recordList"><dl class="orderList"><dd>期号:&nbsp;' + dataSet.IssueNumber + '</dd><dd>中奖金额:&nbsp;<span class="red">' + dataSet.AwardMoney +'</span></dd><dd>彩种:&nbsp;'+LotteryInfo.getLotteryNameById(dataSet.BetTb)+'</dd><dd>时间:&nbsp;' + dataSet.InsertTime +'</dd></dl></a>');
...
function searchBetting_myWinRecord(startDateTime, endDateTime, lotteryType) {
	  page=0;            
    var params = '{"InterfaceName":"GetWindList","ProjectPublic_PlatformCode":2,"LotteryCode":' + lotteryType + ',"IsHistory":' + IsHistory + ',"OrderState":16777216,"insertTimeMin":"' + startDateTime + '","insertTimeMax":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_myWin + '}';
