...
function MyfenHongContractInit() {
    ...
    ajaxUtil.ajaxByAsyncPost1(null, '{"InterfaceName":"GetContract","ProjectPublic_PlatformCode":2,"UserName":""}', function (data) {
        myState = data.State;

        ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"GetContractDetails","ProjectPublic_PlatformCode":2,"UserID":"'+ myUserID +'","IsState":' + myState +'}', getMyfenHongInfo, '正在加载数据...');

    }, '正在加载数据...');
}

//@ 获取我的分红契约数据并显示
function getMyfenHongInfo(dataBack) {
        var list = dataBack.ContractContentModels;
...
function AgreeMyFenHong(IsAgree) {
    ajaxUtil.ajaxByAsyncPost1(null, '{"InterfaceName":"AgreeContract","ProjectPublic_PlatformCode":2,"IsAgree":'+ IsAgree +',"UserName":"'+ myUserName +'","UserID":'+ myUserID +'}', function (data) {
