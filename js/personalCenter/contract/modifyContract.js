...
function modifyContractInit() {
    ...
    ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"GetMyDownContract","ProjectPublic_PlatformCode":2,"UserID":"'+ myUserID +'","DownUserId":' + modifyCttInfo.userID+'}', modifyContract_callBack, '正在加载数据...');
...
function modifyContract_callBack(data) {
    var myContract = data.MyContract;
    var downContract = data.DownContract;
...
function submitModifyContract(modifyCttInfo) {
    ...
    if (modifyCttInfo.State == -1){  //添加契约
        ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"AddContract","ProjectPublic_PlatformCode":2,"UserName":"' + modifyCttInfo.userName +'","ContractContentModels":'+jsonUtils.toString(ContractContent)+'}', function (data) {
            if (data.Result > 0){
        ...
        ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"ModifyContractContent","ProjectPublic_PlatformCode":2,"UserID":'+ modifyCttInfo.userID +',"UserName":"' + modifyCttInfo.userName +'","ContractContentModels":'+jsonUtils.toString(ContractContent)+'}', function (data) {
            if (data.Result){
