...
function MyDailyWagesContractInit() {
    ...
    ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"GetDayWagesDetails","ProjectPublic_PlatformCode":2,"UserID":"'+ myUserID +'"}', function (data) {

        if (data.SystemState == 64){

            var oldActive = data.OldActivePersonNum;  //活跃人数
            var oldStandard = data.OldStandardMoney;  //达标流水
            var myOldData = data.RewardProportion_OldDataModels;  //列表数据

            var newActive = data.NewActivePersonNum;
            var newStandard = data.NewStandardMoney;
            var myNewData = data.RewardProportion_NewDataModels;
...
function AgreeMyDailyWages(IsAgree) {
    ajaxUtil.ajaxByAsyncPost1(null, '{"InterfaceName":"AgreeDayWages","ProjectPublic_PlatformCode":2,"IsAgree":'+ IsAgree +',"UserName":"'+ myUserName +'","UserID":'+ myUserID +'}', function (data) {
