...
function fenHongContract_scroll() {
    ajaxUtil.ajaxByAsyncPost1(null, '{"InterfaceName":"GetContract","ProjectPublic_PlatformCode":2,"UserName":"","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_FenHongCtt + '}', loadfenHongCtt_callBack, '正在加载数据...');
}
...
function loadfenHongCtt_callBack(data){
    ...
    //一键结算 和 footer 是否显示
    data.LockState == 1 ? $("#fenHongContractFooter").parent('.footer').show() : $("#fenHongContractFooter").parent('.footer').hide();

    var info = data.ContractManagerModels;
    if (data.SystemState == 64 && info.length >0){
        ...
        $.each(info,function (key,val) {
            ...
            if (data.LockState == 1 && val.State >= 0){
...
function oneKeyToSettle() {
    ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"OneKeySettlement","ProjectPublic_PlatformCode":2,"UserName":"'+ myUserName +'","Sign":1,"UserID":' + myUserID + '}', function (data) {
    ...
    $.ui.popup({
        ...
        doneCallback:
            function(){
                ...
                page = 0;
                ajaxUtil.ajaxByAsyncPost1(null, '{"InterfaceName":"GetContract","ProjectPublic_PlatformCode":2,"UserName":"'+searchUser+'","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_FenHongCtt + '}', loadfenHongCtt_callBack, '正在加载数据...');
            },
        cancelOnly:false
    });
}
