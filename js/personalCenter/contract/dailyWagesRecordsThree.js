...
function dailyWagesRecordThreeInit() {
    ...
    setStartDate_dailyWages.init({trigger:'#selectDatedailyWages_Stt',min:initDefaultDate(-DayRange_3month,"day"),max:initDefaultDate(0,"day")});
    setEndDate_dailyWages = new MobileSelectDate();
    setEndDate_dailyWages.init({trigger:'#selectDatedailyWages_End',min:initDefaultDate(-DayRange_3month,"day"),max:initDefaultDate(0,"day")});
...
function getdailyWagesRecords_scroll(startDateTime, endDateTime, IsSelf) {
    var param = '{"InterfaceName":"GetDayWagesRecord","ProjectPublic_PlatformCode":2,"IsSelf":'+IsSelf+',"BeginTime":"' + startDateTime + '","EndTime":"' + endDateTime + '","UserID":"'+ myUserID +'","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_dailyWagesRecord + '}';
...
function getdailyWagesRecord_callBack(data){
    ...
    var info = data.GetWayWagesList;
    if (data.SystemState == 64 && info.length >0){
    ...
    $.ui.popup({
        ...
        doneCallback:
            function() {
                ...
                page = 0;
                IsSelf = $("#dailyWagesRecordType").val();
                var param = '{"InterfaceName":"GetDayWagesRecord","ProjectPublic_PlatformCode":2,"User_Name":"' + searchUser + '","BeginTime":"' + startDateTime + '","EndTime":"' + endDateTime + '","IsSelf":' + IsSelf + ',"CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_dailyWagesRecord + '}';
                ajaxUtil.ajaxByAsyncPost1(null, param, getdailyWagesRecord_callBack, null);
            },
        cancelOnly:false
    });
