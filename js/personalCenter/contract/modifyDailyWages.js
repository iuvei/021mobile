...
function modifyDailyWagesInit() {
    ...
    ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"GetMyDownDayWages","ProjectPublic_PlatformCode":2,"UserID":"'+ myUserID +'","SubordinateUserid":' + modifyDayWageInfo.userID+'}', modifyDailyWages_callBack, '正在加载数据...');
...
function modifyDailyWages_callBack(data) {
    var MyDayWages = data.MyDayWages;
    var SubordinateDayWages = data.SubordinateDayWages;
    ...
    if (data.SystemState == 64){
        ...
        $("#modifyDailyWages_mine").append('<p style="text-align:center;">活跃人数：<span class="red">'+ data.ActivePersonNum + '</span>； 达标流水：<span class="red">'+ data.StandardMoney +'</span></p>');
...
function submitModifyDailyWages(modifyDayWageInfo) {
    ...
    if (modifyDayWageInfo.State == 2){  //添加日工资契约
        ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"AddDayWages","ProjectPublic_PlatformCode":2,"UserName":"' + modifyDayWageInfo.userName +'","ActivePersonNum":'+ obj.ActivePersonNum +',"StandardMoney":'+ obj.StandardMoney +',"StatisticalMethod":1,"RewardProportionModels":'+jsonUtils.toString(DailyWagesContent)+'}', function (data) {
