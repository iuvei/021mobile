...
function fenHongSettleInit(){
	...
	ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"GetDividendSettlement","ProjectPublic_PlatformCode":2,"UserID":' + UserID + '}',getfenHongSettle_callBack , '正在加载数据...');

	$("#settleMySubBtn").hide();
	$("#settleMySubBtn").off('click');
	$("#settleMySubBtn").on('click',function () {
		ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"BonusSettlement","ProjectPublic_PlatformCode":2,"DownUserId":' + UserID + ',"DownUserName":"'+ UserName +'"}', settleMySubordinate_callBack , '正在加载数据...');
	});
}
...
function getfenHongSettle_callBack(data) {
	if (data.SystemState == 64){
		$("#fenHongCycle").html(data.DividendCycle);
		var list = data.GetDividendContractList;
		var preview = data.GetBonusPreviewModels;
