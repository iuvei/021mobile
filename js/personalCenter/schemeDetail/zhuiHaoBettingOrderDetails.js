...
function searchOrder_zh(lotteryId, orderId) {
	params='{"InsertTime":"'+insertTime+'","ProjectPublic_PlatformCode":2,"InterfaceName":"GetBetOrderDetil","LotteryCode":"' + lotteryId + '","IsHistory":' + IsHistory + ',"ChaseOrderID":"' + orderId + '"}';
...
function searchSuccessCallBack_zh(data) {
	$("#orderList_zh").empty();
	$("#orderList_zh").append('<li id="list_divider_id" style="background:#c108d7; background:linear-gradient(#9008d7,#c108d7); color:#FFFFFF;padding:6px 16px;font-size: 16px;margin:10px 0 0;">购 买 号 码</li>');
		...
		for (var i = 0; i < bet.length; i++) {
			...
			var $itemdetailLi = $('<li></li>').data('itemdetailLi',dataSet);
               ...
               $itemdetailLi.append('<a href="#" class="recordList"><table><tbody><tr><td><h3 class="whitetext"><span>'
				   ...
				   +'</p><p style="color: #c108d7">'+operateType_+'&nbsp;&nbsp;&nbsp;<span>'+dataSet.qiHao
...
function cancelOrderOneTime(orderID) {
	var params = '{"InterfaceName":"OneKeyCancelOrder","ProjectPublic_PlatformCode":2,"Code":"' + lotteryType + '","OrderID":"' + orderID + '"}';
	ajaxUtil.ajaxByAsyncPost(null, params, function(data){
		if (data.SystemState == 64) {
			if (data.UpdateCompleteNum > 0) {
				toastUtils.showToast("成功撤销 <b>"+ data.UpdateCompleteNum +"</b> 单");
			...
			toastUtils.showToast("撤单失败，请稍后再试");
		}
	}, '撤单中...');
}