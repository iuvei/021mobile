		...
		isHasMorePage(btInfo,PAGESIZE_myBetting);
	 	for (var i = 0; i < btInfo.length; i++) {
			...
			var $itemLi = $('<li></li>').data('proxyscheme',dataSet);
				...
				$itemLi.append('<a class="recordList"><dl class="orderList"><dd>期号:&nbsp;' + dataSet.qiHao + '</dd><dd>投注金额:&nbsp;<span class="red">' + dataSet.tzMoney +'元</span></dd><dd>彩种:&nbsp;'+LotteryInfo.getLotteryNameById(dataSet.lotteryType)+'</dd><dd>时间:&nbsp;' + dataSet.tzTime +'</dd></dl></a>');
...
function searchAllSuccessCallBack_proxy(data) {
	...
	if (data.ErrorState == "0") {
		...
		for (var i = 0; i < Info.length; i++) {
			...
			$("#searchLottery_proxyBettId").append('<option value=' + Info[i].LotteryCode + ' selected="selected">' + LotteryInfo.getLotteryNameById(Info[i].LotteryCode) + '</option>');
			}else{*/
			$("#proxyBettingSearchType").append('<option value=' + Info[i].LotteryCode + '>' + LotteryInfo.getLotteryNameById(Info[i].LotteryCode) +  '</option>');
			// }
		  }
		}
...
function nextPage_searchBetting_proxy(startDateTime, endDateTime, lotteryType) {
    var params = '{"InterfaceName":"GetBetDataListFlex","SourceCode":1,"ProjectPublic_PlatformCode":2,"IsHistory":' + IsHistory + ',"ThisUserName":"' + thisUserName + '","UserID":' + thisUserID + ',"LotteryCode":' + lotteryType + ',"insertTimeMin":"' + startDateTime + '","insertTimeMax":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_myBetting + '}';
