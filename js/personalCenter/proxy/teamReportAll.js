...
function teamReportAllInit(){
    ...
    $("#teamReportAllUlId").empty();
    var $ReportAllLUL=$('<ul class="recordDetail"><li>直 属：<span id="child_all"></span></li><li>团 队：<span id="team_all"></span></li><li>充值总额：<span id="charge_all"></span></li><li>提款总额：<span id="withdrawal_all"></span></li><li>购彩总额：<span id="buyLottery_all"></span></li><li>返点总额：<span id="point_all"></span></li><li>中奖总额：<span id="prize_all"></span></li><li style="display: none;">日工资：<span id="dailywage_all"></span></li><li>其他收入：<span id="others_all"></span></li><li>盈利总额：<span id="winTotal_all"></span></li></ul>');
...
function searchTotal_teamReportAll(startDateTime,endDateTime) {
	ajaxUtil.ajaxByAsyncPost(null, '{"ProjectPublic_PlatformCode":2,"RebateType":1,"ISself":0,"InterfaceName":"Gettotalreport","BeginTime":"' + startDateTime + '","EndTime":"' + endDateTime + '"}', teamReportAllSuccessCallBack, '正在加载数据...');
}
...
function teamReportAllSuccessCallBack(data) {
	if (data.SystemState == 64) {
		$("#charge_all").html(data.ReportComm[0].RechargeTotal);
		$("#withdrawal_all").html(data.ReportComm[0].DrawingsTotal);
		$("#buyLottery_all").html(data.ReportComm[0].BuyTotal);
		$("#point_all").html(data.ReportComm[0].RebateTotal);
		$("#prize_all").html(data.ReportComm[0].WinningTotal);
		$("#winTotal_all").html(data.ReportComm[0].GainTotal);
		// $("#dailywage_all").html(data.ReportComm[0].DailywageTotal);
		$("#others_all").html(bigNumberUtil.add(data.ReportComm[0].OtherTotal,data.ReportComm[0].DailywageTotal).toString());
