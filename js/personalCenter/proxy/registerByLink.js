...
function getRegistUrlListCallBack(data) {
    ...
    if (data.SystemState == 64 ){
        var RegistUrlList = data.UserRegistUrlList;
        ...
        for(var i=RegistUrlList.length-1; i>=0; i--) {
            ...
            if (val.Status == 1){
                list += '注册地址：<span style="font-size: 13px;" class="redtext"> (长按复制)</span><textarea readonly cols="40" rows="2" style="width:100%;color:#FF9e00;font-size:17px;padding:10px 0 0 5px; border: 1px solid #cccccc;margin-bottom:10px;">'+ getShortLink(val.RegistUrl) +'</textarea>';
                ...
                list += '<p style="text-align: right;margin:0;"><button style="background-color: #4fa053;border-color:#4fa053;" onclick="changeLinkStatus('+val.ID+','+val.Status+')">开启链接</button><button onclick="deleteLink('+val.ID+')">删 除</button></p>';
            }
            list += '</li>';
     ...
     //无数据时
        ...
        RegistUrlList = data.UserRegistUrlList;
        if (RegistUrlList.length == 0){
        $("#hasLink").hide();
        $("#noLink").show();
        $("#addNewLink").off('click');
        ...
        $.ui.popup(
            {
                ...
                doneCallback:
                    function(){
                        status = 0;
                        var paramOpen = '{"ProjectPublic_PlatformCode":2,"InterfaceName":"ModifyUserRegistUrl","ID":"'+ ID +'","Status":'+ status +'}';
    ...
    $.ui.popup(
        {
            ...
            doneCallback:
                function(){
                    var paramDelete = '{"ProjectPublic_PlatformCode":2,"InterfaceName":"DeleteUserRegistUrl","ID":"'+ ID +'"}';
