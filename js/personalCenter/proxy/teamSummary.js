...
function teamSummaryRecordsInit(){
	...
	// Init - First time.
	$("#weekChart").css({"backgroundColor":"#c108d7","color":"#fff"});
	$("#weekChart").siblings().css({"backgroundColor":"#f9f9f9","color":"#333"});

	// 查询今日数据
	ajaxUtil.ajaxByAsyncPost(null, '{"ProjectPublic_PlatformCode":2,"InterfaceName":"GetTeamOverViewDayList","UserID":"' + myUserID + '"}', teamSumSuccessCallBack_today, null);
	...
	$("#changeChartID button").on('click',function () {
		$("#lineChart").empty();
		$(this).css({"backgroundColor":"#c108d7","color":"#fff"});
...
function searchTotal_teamSummary(startDateTime, endDateTime, dataType) {
		ajaxUtil.ajaxByAsyncPost(null, '{"ProjectPublic_PlatformCode":2,"InterfaceName":"GetTeamOverViewChartList","UserID":"' + myUserID + '","DataType":'+ dataType +',"BeginTime":"' + startDateTime + '","EndTime":"' + endDateTime + '"}', teamSumSuccessCallBack_chart, null);
}
...
function teamSumSuccessCallBack_chart(data) {
	...
	if (data.SystemState == 64) {
		...
		if (data.TeamDataInfoList.length >0 && dataType == 0){
			// 团队充值
			addDatatoArray(charge, data.TeamDataInfoList);
		}else if(data.TeamDataInfoList.length >0 && dataType == 1){
			// 团队提款
			addDatatoArray(withdrawal,data.TeamDataInfoList);
		}else if(data.TeamDataInfoList.length >0 && dataType == 2){
			// 团队投注
			addDatatoArray(teamBet, data.TeamDataInfoList);
		}else if(data.TeamDataInfoList.length >0 && dataType == 3){
			// 注册人数
			addDatatoArray(register, data.TeamDataInfoList);
		}else if(data.TeamDataInfoList.length >0 && dataType == 4){
			// 投注人数
			addDatatoArray(betNum, data.TeamDataInfoList);
		}else if(data.TeamDataInfoList.length >0 && dataType == 5){
			// 登录人数
			addDatatoArray(online, data.TeamDataInfoList);
		}
	...
	changeChart(charge, withdrawal, teamBet, register, betNum, online);
}
...
//@ Change chart.
function changeChart(DataCharge, DataWithdraw, DataBet, DataRegister, DataBetNum, DataOnline) {
	var DataArray = [DataCharge, DataWithdraw, DataBet, DataRegister, DataBetNum, DataOnline]; //元素顺序Important
...
function showChart(xAxis,dataArray) {
	//Analysis of Data
	var dataName = ['团队充值','团队提款','团队投注','注册人数','投注人数','登录人数']; //dataName.length = datArray.length
	var seriesArray = [];
	//设置默认选中项
	var legendSelected = {
		'团队充值': false,
	...
	var option = {
		...
		color:['#FF9e00 ','#FF9e00','#FF9e00','#FF9e00','#FF9e00','#FF9e00'],
		//数据区域缩放
		dataZoom: [{
				type: 'slider',  //X轴滑动条
				start: 0, // %
