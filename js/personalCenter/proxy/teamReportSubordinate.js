...
function searchteamReportSubordinate_Record(startDateTime, endDateTime, type) {
    var temp = localStorageUtils.getParam("myUserID");
    if(uid != temp){//下级
        ajaxUtil.ajaxByAsyncPost(null, '{"User_ID":"'+uid+'","RebateType":1,"InterfaceName":"GetTotalChildReport","ProjectPublic_PlatformCode":2,"BeginTime":"' + startDateTime + '","EndTime":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_teamReportSubordinate + '}', teamReportSubordinate_searchSuccessCallBack, '正在加载数据...');
    }else{
        ajaxUtil.ajaxByAsyncPost(null, '{"UserName":"","User_ID":"'+uid+'","RebateType":1,"InterfaceName":"GetTotalChildReport","ProjectPublic_PlatformCode":2,"GetUserType":' + type + ',"BeginTime":"' + startDateTime + '","EndTime":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_teamReportSubordinate + '}', teamReportSubordinate_searchSuccessCallBack, '正在加载数据...');
    }
}
/**
 *查询团队报表/下级记录
...
function queryteamReportSubordinateUserNameIdUserName(searchUser){
    page = 0;
    ajaxUtil.ajaxByAsyncPost(null, '{"User_ID":"'+uid+'","UserName":"' + searchUser + '","InterfaceName":"GetTotalChildReport","ProjectPublic_PlatformCode":2,"GetUserType":' + type + ',"BeginTime":"' + startDateTime + '","EndTime":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_teamReportSubordinate + '}', 
    teamReportSubordinate_searchSuccessCallBack, '正在加载数据...');
}
