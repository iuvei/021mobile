...
// <select name="searchDate_teamBetting" id="searchDate_teamBetting" data-theme="a" data-mini="true" onchange="dateChange_teamBetting()"><option value="0" selected="selected">当前记录</option><option value="1">历史记录</option></select>
    ...
    var $select=$('<table><tr>' +
        ...
        '<tr><td colspan="3"><select name="searchStatus_teamBetting" id="searchStatus_teamBetting" data-theme="a" data-mini="true" onchange="statusChange_teamBetting()"><option  selected="selected" value="-1">状态：全部</option>'+
...
function createTeamBettingRecordsList(data){
    ...
    if (data.SystemState == 64 && data.DataCount !=0) {
        ...
        for (var i = 0; i < btInfo.length; i++) {
            ...
            $itemLi.append('<a class="recordList"><dl class="orderList"><dd>用户名:&nbsp;' + dataSet.UserName + '</dd><dd>投注金额:&nbsp;<span class="red">' + scheme.tzMoney +'元</span></dd><dd>彩种:&nbsp;'+LotteryInfo.getLotteryNameById(dataSet.BetTb)+'</dd><dd>时间:&nbsp;' + dataSet.InsertTime +'</dd></dl></a>');
...
function nextPage_teamBetting(startDateTime, endDateTime, lotteryType) {
    var params = '{"InterfaceName":"GetTeamBetDataListNew","SourceCode":1,"ProjectPublic_PlatformCode":2,"LotteryCode":' + lotteryType + ',"IsHistory":' + IsHistory + ',"OrderState":"'+ orderStatus +'","insertTimeMin":"' + startDateTime + '","insertTimeMax":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_teamBetting + '}';
...
function queryTeamBettingUserName(searchUser){
    page = 0;
    ajaxUtil.ajaxByAsyncPost(null, '{"LikeUserName":"'+searchUser+'","InterfaceName":"GetTeamBetDataListNew","LotteryCode":' + lotteryType + ',"SourceCode":1,"ProjectPublic_PlatformCode":2,"IsHistory":' + IsHistory + ',"OrderState":"'+ orderStatus +'","insertTimeMin":"' + startDateTime + '","insertTimeMax":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_teamBetting + '}',
