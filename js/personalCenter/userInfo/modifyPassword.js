...
function modifyPasswordInit(){
	...
	if (LoginPwdForced == 'true'){
		...
		$('#oldPassword').hide();
		$('#oldPassword').prev('p').html('温馨提示：您的登录密码为默认密码，请修改后重新登录！').css({'color':'#FF9e00','padding':'5px 10px'});
	}else {
	...
	$("#loginSubmitId").on('click', function() {
		...
		//Type:0 -- 登录和资金密码可以一致，Type:1 -- 登录和资金密码不可一致。
		 var param = '{"ProjectPublic_PlatformCode":2,"Type":1,"UserName":"' + userName + '","LogPassword":"' + oldPassword + '","NewPassword":"' + confirmPassword + '","InterfaceName":"ModifyPwdInfo"}';
