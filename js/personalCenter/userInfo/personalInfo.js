...
function personalInfoInit(){
	...
	ajaxUtil.ajaxByAsyncPost1(null, paramSecurity, securityCallBack_personaInfo,null);

	// 修改个人资料
	$("#change_personal_infoId").off('click');
	$("#change_personal_infoId").on('click', function(){
		createInitPanel_Fun('modifyInfo');

	});

    // 修改银行资料
    $("#change_bankId").off('click');
    $("#change_bankId").on('click', function(){
	    createInitPanel_Fun('showBankInfo');
    });	    
    // 修改登录密码
    $("#change_login_passwordId").off('click');
    $("#change_login_passwordId").on('click', function(){
		createInitPanel_Fun('modifyPassword');
    });

    // 修改资金密码
    $("#change_withdraw_passwordId").off('click');
    $("#change_withdraw_passwordId").on('click', function(){
		if (securityState){
	...
	// 密保设置 ( 1：已设置; 0：未设置 )
	$("#set_security").off('click');
	$("#set_security").on('click', function(){
		loadSetSecurityPage('setSecurity');
	});
}
 ...
 * @return data 服务端返数据
 */
function successCallBack_personaInfo(data) {
	if (data.SystemState == 64) {
		...
		$("#user").html(userName);
        if(realName){
			$("#realName").html(realName.substring(1,0)+"**");
        }else{
			$("#realName").html("");
        }

		$("#userFandian").html(userFandian);
...
//查询是否有提款密码
function searchSuccessCallBack_personaInfo(data) {
	if (data.Result == "1") {
...
function securityCallBack_personaInfo(data) {
	if (data.SystemState == 64){
		securityState = data.Result;
		if (securityState){
			$("#set_security").html("密保设置 （已设置）<span class='btnRight'></span>");
		}else{
			$("#set_security").html("密保设置 （未设置）<span class='btnRight'></span>");
		}
...
function loadSetSecurityPage(nextPage) {
	if (securityState){
		var Security = '{"ProjectPublic_PlatformCode":2,"InterfaceName":"GetRandomSecurity","UserID":' + myUserID + ',"FlagType":1,"SecurityCount":2}';
