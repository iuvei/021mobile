...
function modifyBankInfoInit(){
	$("#bank").empty();
	$("#bank").append('<option value="noselect">请选择银行</option><option value="icbc">中国工商银行</option><option value="abc">中国农业银行</option><option value="ccb">中国建设银行</option><option value="cmb">招商银行</option><option value="comm">交通银行</option><option value="boc">中国银行</option><option value="ceb">中国光大银行</option><option value="citic">中信银行</option><option value="cib">兴业银行</option><option value="psbc">中国邮政银行</option><option value="hxb">华夏银行</option><option value="spdb">上海浦东发展银行</option><option value="bob">北京银行</option><option value="gdb">广发银行</option><option value="gzb">广州银行</option><option value="hzb">杭州银行</option><option value="bos">上海银行</option><option value="sdb">平安银行</option><option value="czb">浙商银行</option><option value="bod">东莞银行</option><option value="cbhb">渤海银行</option><option value="cmbc">民生银行</option>');
	...
	$("#submit").on("click", function() {
		...
		$.ui.popup(
			{
				title:"确认银行资料",
				message:'<p style="line-height: 22px;text-align:left;padding-left:15%;">持卡人姓名 : <i class="redtext">'+ $("#userRealName").val() +'</i><br>开户银行 : <i class="redtext">'+ bankValue[userBankVal].name+'</i><br>卡号 : <i class="redtext">'+userBankNoVal +'</i>'+ showBankLoca(userProv,userCity) + showBankSub(subBranch)+'</p>',
				cancelText:"取消",
				cancelCallback:
					function(){
					},
